...
export default function Home() {
  ...
  async function handleAddKeyword(keyword: string) {
    ...
    try {
      const response = await fetch('/api/monitoring', {
        ...
        body: JSON.stringify({ action: 'add', type: 'keyword', keyword: normalizedKeyword }),
      });
  ...
  async function handleDeleteKeyword(keyword: string) {
    setKeywords((currentKeywords) => currentKeywords.filter((currentKeyword) => currentKeyword !== keyword));
    ...
    try {
      const response = await fetch('/api/monitoring', {
        ...
        body: JSON.stringify({ action: 'delete', type: 'keyword', keyword }),
      });

      if (!response.ok) {
        throw new Error('关键词删除失败');
      }

      const payload = (await response.json()) as DeleteResponse;
      setDataMessage(payload.source === 'supabase' ? '关键词已从 Supabase 删除。' : 'Supabase 未配置，关键词已从本地状态删除。');
    } catch (error) {
      const message = error instanceof Error ? error.message : '关键词删除失败';
      setKeywords((currentKeywords) => (currentKeywords.includes(keyword) ? currentKeywords : [...currentKeywords, keyword]));
      setDataMessage(`${message}，已恢复本地关键词。`);
    }
  }
  ...
  async function handleDeleteAccount(accountId: string) {
    ...
    try {
      const response = await fetch('/api/monitoring', {
        ...
        body: JSON.stringify({ action: 'delete', type: 'account', id: accountId }),
      });

      if (!response.ok) {
        throw new Error('指定账号删除失败');
      }

      const payload = (await response.json()) as DeleteResponse;
      setDataMessage(payload.source === 'supabase' ? '指定账号已从 Supabase 删除。' : 'Supabase 未配置，指定账号已从本地状态删除。');
    } catch (error) {
      const message = error instanceof Error ? error.message : '指定账号删除失败';
      setAccounts((currentAccounts) => (currentAccounts.some((account) => account.id === accountId) ? currentAccounts : [accountToDelete, ...currentAccounts]));
      setDataMessage(`${message}，已恢复本地账号。`);
    }
  }
