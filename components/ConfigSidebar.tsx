...
function AccountRow({ account, onDelete }: { account: AccountItem; onDelete: (accountId: string) => void }) {
  return (
    <div className="rounded-2xl border border-slate-100 p-3">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="text-xs font-bold text-slate-400 transition hover:text-red-600" type="button" onClick={() => onDelete(account.id)}>
            删除
          </button>
        </div>
      </div>
      ...
      <Card title="监控关键词">
        <div className="flex flex-wrap gap-2">
          {keywords.map((keyword) => (
            <span key={keyword} className="inline-flex items-center gap-2 rounded-full bg-slate-100 px-3 py-2 text-sm font-semibold text-slate-700">
              {keyword}
              <button className="text-xs font-bold text-slate-400 transition hover:text-red-600" type="button" onClick={() => onDeleteKeyword(keyword)} aria-label={`删除关键词 ${keyword}`}>
                ×
              </button>
            </span>
          ))}
