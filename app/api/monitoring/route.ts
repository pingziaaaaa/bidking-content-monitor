...
export async function GET() {
  ...
  try {
    ...
    const contentsQuery = db
      .from('content_items')
      .select('id, platform, title, url, creator, followers, source, discovered_at, views, impressions, engagements, peak_viewers, vod_views, created_at')
    ...
    let resolvedContentsResult: SupabaseListResult<ContentRow> = contentsResult;

    if (contentsResult.error?.message.toLowerCase().includes('followers')) {
      resolvedContentsResult = await db
        .from('content_items')
    ...
    if (resolvedContentsResult.error || keywordsResult.error || accountsResult.error) {
      throw new Error(resolvedContentsResult.error?.message || keywordsResult.error?.message || accountsResult.error?.message || 'Supabase 查询失败');
    }

    const contentRows: ContentRow[] = rowsFromSupabase(resolvedContentsResult.data);
